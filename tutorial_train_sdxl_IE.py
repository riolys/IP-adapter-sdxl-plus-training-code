...
def main():
    ...
    # Freeze parameters of models to save more memory, except image_encoder
    unet.requires_grad_(False)
    vae.requires_grad_(False)
    text_encoder.requires_grad_(False)
    text_encoder_2.requires_grad_(False)
    # Commented out to include image_encoder in training
    ...
    # Note: When using accelerator.prepare, it's better not to manually move models to devices
    # Remove manual .to() calls for vae and text encoders if you include them in prepare
    vae.to(accelerator.device) # use fp32
    text_encoder.to(accelerator.device, dtype=weight_dtype)
    text_encoder_2.to(accelerator.device, dtype=weight_dtype)
    image_encoder.to(accelerator.device, dtype=weight_dtype)  # Previously frozen, now trainable
    
    # Define optimizer parameters to include image_encoder
    params_to_opt = itertools.chain(
        ip_adapter.image_proj_model.parameters(),
        ip_adapter.adapter_modules.parameters(),
        image_encoder.parameters()  # Include image_encoder parameters
    )
    ...
    # Prepare everything with `accelerator`, including image_encoder
    ...
    for epoch in range(0, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(ip_adapter):
                ...
                if accelerator.is_main_process:
                    print("Epoch {}, step {}, data_time: {:.4f}s, step_time: {:.4f}s, step_loss: {:.6f}".format(
